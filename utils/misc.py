...
def args_parser():
    ...
    parser.add_argument('--poison', action='store_true', help='poison or not')
    parser.add_argument('--adversary_frac', type=float, default=0.2)
    parser.add_argument('--scale_up', type=float, default=1.0)

    parser.add_argument('--attack_epsilon', type=float, default=1.0)
    parser.add_argument('--num_ensemble', type=int, default=2)
    parser.add_argument('--attack_rounds', type=int, default=20)
    parser.add_argument('--attack_optimizer', type=str, default='adam')
    parser.add_argument('--surrogate_model', type=str, default='lstm')
    parser.add_argument('--attack_lr', type=float, default=10.0)
    ...
    ### apply defenses
    parser.add_argument('--apply_defense', type=str, default=None,
                            choices=[None, 'sphere_sani', 'rand', 'adj_sani',
                                        'multi_krum', 'trimmed_mean', 'median'])
    parser.add_argument('--apply_detection', action='store_true')
    parser.add_argument('--removal_proportion', type=float, default=0.2)
...
def set_logger(log_id, args, logger_name = None):
    """Set the logger to log info in terminal and log file.
    ...
    Args:
        log_id: (string) where to log
    """

    if args.poison:
        if args.apply_defense is not None:
            out_dir = 'poison_attacks_defended_by_{}'.format(args.apply_defense)
        else:
            out_dir = 'poison_attacks'
    else:
        out_dir = 'standard'
    if args.apply_detection:
        out_dir += '_apply_detection'

    if args.poison:
        if 'Cen' in log_id:
            log_id += '--epsilon-{:}-advfrac-{:}-advensemble-{}-advrounds-{}-advopt-{}'.format(args.attack_epsilon,
                                                                                              args.adversary_frac,
                                                                                              args.num_ensemble,
                                                                                              args.attack_rounds,
                                                                                              args.attack_optimizer)

            log_id += '--maskprob-{}-attacklr-{}-surrogate-{}'.format(args.mask_prob, args.attack_lr, args.surrogate_model)

            if args.apply_defense is not None and 'sani' in args.apply_defense:
                log_id += '-remove_pro-{}'.format(args.removal_proportion)

            log_id += '-rounds-{}'.format(args.epochs)


        elif 'Fed' in log_id:
            log_id += '--epsilon-{:}-advfrac-{:}-scaleup-{}'.format(args.attack_epsilon,
                                                                  args.adversary_frac,
                                                                  args.scale_up)
            if args.apply_detection:
                log_id += '-rounds-{}'.format(args.epochs)
    ...
    log_file = os.path.join(out_dir, '{}.log'.format(log_id))
...
def save_model(log_id, args, state, selected_cells, adv_cells=None):
    ...
    filepath = os.path.join(out_dir, '{}.pth'.format(log_id))
    torch.save(state, filepath)

    with open(os.path.join(out_dir, '{}.pkl'.format(log_id)), 'wb') as fid:
        if args.poison:
            pickle.dump((selected_cells, adv_cells), fid)
        else:
            pickle.dump(selected_cells, fid)


def save_poison_data(log_id, data):
    out_dir = 'poison_attacks_data'
    if not os.path.exists(out_dir):
        os.mkdir(out_dir)
...
def load_poison_data(log_id, args):
    out_dir = 'poison_attacks_data'
    ...
    with open(os.path.join(out_dir, '{}.pkl'.format(log_id)), 'rb') as fid:
