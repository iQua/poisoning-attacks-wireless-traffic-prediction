...
class CNNLSTM(nn.Module):
    def __init__(self, args):
        ...
        self.input_dim = args.input_dim
        self.out_channels = args.hidden_dim//4
        self.hidden_dim = args.hidden_dim
        self.out_dim = args.out_dim
        self.num_layers = args.num_layers
        self.close_size = args.close_size
        ...
        self.conv = nn.Conv1d(in_channels=self.input_dim, out_channels=self.out_channels, kernel_size=3, padding=1)

        self.lstm_close = nn.LSTM(input_size=self.out_channels, hidden_size=self.hidden_dim, num_layers=self.num_layers,
                                  batch_first=True, dropout=0.2)
        self.lstm_period = nn.LSTM(input_size=self.out_channels, hidden_size=self.hidden_dim, num_layers=self.num_layers,
                                   batch_first=True, dropout=0.2)
